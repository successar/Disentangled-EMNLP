...
% bcw 2/19 : removed for space, for now. i think can safely assume people familiar w/cosine sim.
\begin{comment}
\begin{equation}
\vspace{-.35em}
\text{\it sim}_a(i, j) = \frac{\mathbf{e}^a_i \cdot \mathbf{e}^a_j}{|\mathbf{e}^a_i||\mathbf{e}^a_j|}.
%\vspace{-.1em}
\end{equation}
\end{comment}
...
%\vspace{-.25em}
The preceding setup assumes a somewhat unique case in which we have access to aspect-specific summaries written for texts. As a more general setting, we also consider learning directly from triplet-wise supervision concerning relative similarity with respect to particular aspects \cite{amid2015multiview,veit2017conditional,wilber2014cost}. The assumption is that such judgments can be solicited directly from annotators, and thus the approach may be applied to arbitrary domains, so long as meaningful aspects can be defined implicitly via pairwise similarities regarding them. In practice, we do not currently have corpora with such judgments in NLP, so we constructed two datasets using aspect-specific sentiment ratings. % bcw: TODO elaborate ... 
...
\vspace{.15em}
\noindent \textbf{Results}. Table \ref{table:cohenauc} reports the mean summary AUC over the set of reviews in the \newcite{cohen2006reducing} corpus. We report detailed results (on each individual review) in the Appendix. In brief: The proposed PICO embeddings obtain an equivalent or higher AUC than baseline strategies on 12/14 reviews, and strictly higher AUCs in 11/14. It is unsurprising that we outperform unsupervised approaches, but we also best RR-TF, which was trained with the same CDSR corpus \cite{wallace2016extracting}, and DSSM \cite{shen2014latent}, which exploits the same triplet loss as our model. Indeed, we outperform the latter by a mean performance gain of 4 points AUC. 
...
%the study I vectors do not cluster with trial populations, aligning with our prior expectations as these involve the same category of intervention. %The O vectors exhibit some clustering which is reasonable because studies of similar populations will often use similar outcome measures. This result can be quantitatively measured by computing the AUC values for the DA dataset using only P, I/C or O embeddings. In Table \ref{table:AUCs-DA}, We observe that the population embeddings give higher AUC score than I/C and O embeddings which is reasonable since population is the discriminative factor for this dataset.
